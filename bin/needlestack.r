        ...
        for (cur_del in uniq_del) {
          ...
          if (!is.na(reg_res$coef["slope"]) & sum(reg_res$GQ>=GQ_threshold,na.rm=TRUE)>0) {
            ...
            if (sum(sbs<=SB_threshold_indel & reg_res$GQ>=GQ_threshold,na.rm=TRUE)>0) {
              ...
              cat(linepos[1],"\t",as.numeric(linepos[2])+1,"\t",".","\t",cur_del,"\t","-","\t",max(reg_res$GQ),"\t",".",sep = "",file=out_file,append=T)
              # INFO field
        ...
        for (cur_ins in uniq_ins) {
          ...
          if (!is.na(reg_res$coef["slope"]) & sum(reg_res$GQ>=GQ_threshold,na.rm=TRUE)>0) {
            ...
            if (sum(sbs<=SB_threshold_indel & reg_res$GQ>=GQ_threshold,na.rm=TRUE)>0) {
              ...
              close(con)
              prev_bp=substr(before,3,3)
              cat(linepos[1],"\t",linepos[2],"\t",".","\t","-","\t",cur_ins,"\t",max(reg_res$GQ),"\t",".",sep = "",file=out_file,append=T)
              # INFO field
              ...
              if (do_plots=="ALL") { #output all variants
                ...
                if(do_alignments==TRUE){ #add alignment plots
                  ...
                  if(isTNpairs){
                    ...
                    pdf(paste(linepos[1],"_",linepos[2],"_",as.numeric(linepos[2]),"_","-","_",cur_ins,ifelse(ref_inv,"_inv_ref",""),ifelse(reg_res$extra_rob,"_extra_robust",""),".pdf",sep=""),9,8)
                    par(mar=c(8,7,8,7))
                  }
                  #add regression plots to the pdf output
