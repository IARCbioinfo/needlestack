        ...
        for (cur_del in uniq_del) {
          ...
          if (!is.na(reg_res$coef["slope"]) & sum(reg_res$GQ>=GQ_threshold,na.rm=TRUE)>0) {
            ...
            if (sum(sbs<=SB_threshold_indel & reg_res$GQ>=GQ_threshold,na.rm=TRUE)>0) {
              ...
              if(!ref_inv & nchar(cur_del)>10) cur_del = paste("DEL",nchar(cur_del),sep="") #paste(substr(cur_del,1,5+match(cur_del,uniq_del)),substr(cur_del,nchar(cur_del)-(5+match(cur_del,uniq_del)),nchar(cur_del)),sep="...")
