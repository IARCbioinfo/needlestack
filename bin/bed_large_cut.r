...
#bed=read.table("wes_bed_merged.bed",stringsAsFactors = F)
bed=read.table(pipe('cat /dev/stdin'),stringsAsFactors = F)
bed_cum_size=cumsum(bed[,3]-bed[,2])
cut_cum_size=round(seq(1,bed_cum_size[length(bed_cum_size)],l=nb_pieces+1))[-c(1)]
lines_cut_bed=unique(c(0,unlist(lapply(cut_cum_size,function(x){min(which(bed_cum_size>=x))}))))
size=numeric(nb_pieces)
