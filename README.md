...
|-----------|---------------|-----------------|
| `bam_folder`    |     `BAM/` | Folder containing the BAM files |
...
| `max_prop_extra_rob` | `0.1` | Maximum proportion of samples having an allelic fraction to be excluded from extra-robust regression |
...
1. `--plots`:
	* `SOMATIC`: produce pdf regression plots (like in the description above) only for somatic variants. Default value when using matched tumor/normal.
